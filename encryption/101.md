...
I did described the process on the high-level, and the person asking even looked satisifed with my answer. But I felt that my explanation was vague and has a few blind spots. So I decide to study the topic myself and give it another try, this time in writing. In this attempt besides clear general explanation of the concept I aim to explain __exactly__ how it plays out in the real life.

> Besides that I'll try to give you an explanation for the words you may face when studying this or a related topic. The words we'll need right away are **bits** and **binary**. Bit is a thing that can hold exactly two states - 0 or 1 for example. So we might, as well, say that computers talk in bits. And since a bit sequence is a sequence of thing that can be one of two the sequence is called binary sequence, or simply **binary**.

## Creating an encoding

Perhaps, the best way to understand something is to build it. That's exactly what we are going to do. We'll create a system to convert the string into a binary sequence a.k.a. **encode** the string, and then receiving the binary sequence convert it back to the string a.k.a **decode** the binary sequence. Unoriginally, the types of "systems" called **encoding**.

For simplicity sake let's limit our alphabet for 8 symbols.s
